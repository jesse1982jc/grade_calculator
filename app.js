...
function mergeSort(arr) {
  ...
  if (arr.length == 1) {
    ...
    let middle = Math.floor(arr.length / 2); // middle 是一個整數，把陣列除2，然後無條件捨去
    let left = arr.slice(0, middle); // left 是 arr 的左半邊陣列
    let right = arr.slice(middle, arr.length); // right 是 arr 的右半邊陣列
    return merge(mergeSort(left), mergeSort(right)); // 遞迴
    // 外面括號做 arr1 與 arr2 比大小
    // 裡面左半邊陣列再細切一半，一直切 (遞迴)
    // 裡面右半邊也是陣列再細切一半，一直切 (遞迴)
  }
}
